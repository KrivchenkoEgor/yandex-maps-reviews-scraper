...
## ✨ Что умеет парсер отзывов Яндекс Карт — проверено на живых данных

Скачивает отзывы Яндекс Карт в Excel / CSV / JSON — парсер Яндекс Бизнес (Yandex Maps reviews scraper) выгружает все отзывы по ссылке на организацию с полной пагинацией через официальный `fetchReviews API`.

**Проверено на реальных магазинах:**
- **Магнит — 222 отзыва** за 5 страниц `pageSize 50` (50×4 + 22). Пагинация `fetchReviews?businessId=...&page=1..5&pageSize=50` работает стабильно.
- **Добрянка — 600 отзывов** за 12 страниц и **сеть Добрянка — 1750 отзывов** за 35 страниц. Лимит 600 на точку — ограничение сортировки Яндекс Карт.
- **Анализ фото:** только `get-altay` (аватарки `get-yapic/islands-68` отфильтрованы). Поддержка `background-image` и `data-srcset`.
- **Полный текст без обрезки:** клик «Ещё» (до 12 кликов) + слияние с API по `reviewId` — в Excel попадает полный текст.
- **Выгрузка через API, а не скроллинг:** перехват `fetchReviews API Яндекс Карт` + имитация пользователя. Стабильнее парсинга верстки.
- **Обход защиты Яндекс (антибан):** паузы, `networkidle`, определение капчи и заглушки `limited`.
- **Кэширование и мониторинг конкурентов:** кэш `SQLite` на 24 часа по `businessId` — повторная выгрузка без запросов. Для мониторинга отзывов и репутационного анализа.
- **Готовый отчёт для аналитики:** автоэкспорт в `Excel (3 листа: Отзывы, Статистика, Магазин) + CSV (utf-8-sig) + JSON` в `output/`.
